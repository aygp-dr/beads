...
func checkDiskSpace(path string) (uint64, bool) {
	...
	// Bavail type varies by OS (uint64 on Linux, int64 on FreeBSD/Darwin)
	// Bsize is int64; overflow is intentional/safe in this context
	availableBytes := uint64(stat.Bavail) * uint64(stat.Bsize) //nolint:gosec
